...
public class KeyPropertyEditor extends AbstractPropertyEditor<Key> {
    ...
    public Key getValue() {
        ...
        if (parentKeyObject.isObject() != null) {
            parentKey = Key.fromJsonObject(parentKeyObject.isObject());
        }

        return new Key(kind.getText(), name.getText(), appId.getText(), id.getValue(),
                new AppIdNamespaceDto(appIdNamespace.getText(), namespace.getText()), parentKey);
    }
